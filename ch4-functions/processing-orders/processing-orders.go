...
func placeOrder(productID string, quantity int, accountBalance float64) (bool, float64) {
	...
	var orderCost = 0.0
	var accountBalanceCheck = -1.0
	if amountInStock(productID) >= quantity {
		orderCost = priceList(productID) * float64(quantity)
		accountBalanceCheck = accountBalance - orderCost
	}
	if accountBalanceCheck >= 0 {
		orderSuccess = true
		accountBalance = accountBalanceCheck
	}
